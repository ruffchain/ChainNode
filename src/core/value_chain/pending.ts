import * as BasePending from '../chain/pending';
...
export class PendingTransactions extends BasePending.PendingTransactions {
    ...
    protected async getStorageBalance(s: string): Promise<BigNumber> {
        try {
            let kvr = await this.m_storageView!.getReadableKeyValue(Chain.kvBalance);
    ...
    protected addToQueue(txTime: BasePending.TransactionWithTime) {
        let pos: number = 0;
        for (let i= 0; i< this.m_transactions.length; i++) {
            if (this.m_transactions[i].tx.address === txTime.tx.address) {
                ...
                pos = (this.m_transactions[i].tx as Transaction).fee.lt((txTime.tx as Transaction).fee)? i : i+1;
            }
        }
