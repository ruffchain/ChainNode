import {Chain, LoggerInstance, ErrorCode, stringifyErrorCode, EventLog} from '../../core';
import {ChainEventStorage} from './storage';
import {ChainEventFilterStub} from './stub';

import { isNullOrUndefined, isString, isObject } from 'util';
import * as path from 'path';

export class ChainEventServer {
    constructor(options: {
        chain: Chain
    }) {
        ...
        this.m_storage = new ChainEventStorage({
    ...
    async init(options: {readonly?: boolean}): Promise<ErrorCode> {
        const readonly = isNullOrUndefined(options.readonly) ? false : options.readonly;
        const sir = await this.m_storage.init({
            readonly
        });
        if (readonly || (sir.err && sir.err !== ErrorCode.RESULT_NOT_FOUND)) {
            return sir.err;
        }
        if (sir.err === ErrorCode.RESULT_NOT_FOUND) {
            const ghr = await this.m_chain.getHeader(0);
            if (ghr.err) {
                this.m_logger.error(`event server init failed for get genesis header failed ${stringifyErrorCode(ghr.err)}`);
                return ghr.err;
            }
            ...
            if (err) {
                this.m_logger.error(`event server init failed for add genesis to storage failed ${stringifyErrorCode(err)}`);
                return err;
            }
        }
        this._syncEvents();
    ...
    private async _syncEvents() {
        ...
        while (true) {
            ...
            if (lbr.err) {
                this.m_logger.error(`event server sync failed for get latest block from storage ${stringifyErrorCode(lbr.err)}`);
                break;
            }
            const ghr = await this.m_chain.getHeader(lbr.latest!.number + 1);
            if (ghr.err) {
                if (ghr.err === ErrorCode.RESULT_NOT_FOUND) {
                    this.m_logger.info(`event server sync ignored for no more block`);
                } else {
                    this.m_logger.error(`event server sync failed for get header from chain ${stringifyErrorCode(ghr.err)}`);
                }
                break;
            }
            if (ghr.header!.preBlockHash === lbr.latest!.hash) {
                ...
                if (err) {
                    this.m_logger.error(`event server sync failed for add block ${block.hash} to storage failed ${stringifyErrorCode(err)}`);
                    break;
                }
            } else {
                let _ghr = await this.m_chain.getHeader(lbr.latest!.hash);
                if (_ghr.err) {
                    this.m_logger.error(`event server sync failed for get header ${lbr.latest!.hash} from chain failed ${stringifyErrorCode(_ghr.err)}`);
                    break;
                }
                let forkFrom = _ghr.header!;
                let _err = ErrorCode.RESULT_OK;
                while (true) {
                    _ghr = await this.m_chain.getHeader(forkFrom.number - 1);
                    if (_ghr.err) {
                        this.m_logger.error(`event server sync failed for get header ${forkFrom.number - 1} from chain failed ${stringifyErrorCode(_ghr.err)}`);
                        _err = _ghr.err;
                        break;
                    }
    ...
    private m_storage: ChainEventStorage;
